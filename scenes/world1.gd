...
func _ready():
	...
	if menu:
		menu.visible = false
		menu.z_index = 1000
		call_deferred("move_child", menu, get_child_count() - 1)
...
func render_puzzle():
	pause_game()
	var scene := preload("res://scenes/puzzle_mapa1.tscn")
	var p := scene.instantiate()
	var theme := preload("res://resources/chain_problems/new_resource.tres") 
	assert(theme is ChainTheme)
	p.chain = theme 
	add_child(p)
...
func _open_win_screen():
	pause_game()
	if menu:
		menu.visible = false  # evita sobreposição do menu
